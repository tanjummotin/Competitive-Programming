...
///Given an array size N and Q queries of form L R K,find elements int the range L o R which are strictly smaller than K
#include<bits/stdc++.h>
using namespace std;
...
void init(ll node,ll b,ll e)
{
    ...
    while(i<tree[left].size() && j<tree[right].size())
    {
        ...
        {
            tree[node].push_back(tree[right][i]);
            j++;
        }
    }
...
ll query(ll node,ll b,ll e,ll i,ll j,ll k)
{
    ...
    if (b >= i && e <= j)
    {
        ll res=upper_bound(tree[node].begin(),tree[node].end(),k-1)-tree[node].begin();
        return res;
    }
